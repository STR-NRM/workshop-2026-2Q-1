...
export const QUESTION_VERSION = '2026-05-12-v1.1';
...
const scale = (id, section, title, question, helpText, tag) =>
  q({
    id,
    type: 'scale5na',
    section,
...
export const questions = [
  ...
  q({
    ...
    question:
      '최근 6주 동안 가장 많이 관여한 제품 또는 워크스트림을 선택해주세요. 최대 2개까지 선택할 수 있습니다.',
    helpText:
      '본인의 업무 시간을 기준으로 답해주세요. 이 문항은 제품별 책임 범위와 우선순위 병목을 해석하기 위한 보조 정보입니다.',
    options: [
      'AI Agent',
      'AI Viewer',
      'AI Idea',
  ...
  q({
    ...
    options: ['3개월 미만', '3~6개월', '6개월 이상', '응답하지 않음'],
  }),
  q({
    id: 'META_EXTERNAL',
    type: 'multiChoice',
  ...
  q({
    ...
    options: ['예', '아니오', '판단하기 어려움', '응답하지 않음'],
  }),
  ...
  scale('B01', 'B. 고객, 사용자, 내부 이해관계자 피드백 루프', '기관 고객 요구 반영', '도서관/기관 고객의 요구와 제약이 제품 의사결정에 충분히 반영된다.', '구매자 관점의 요청, 계약/도입 조건, 운영 제약이 제품 판단에 들어오는지 기준으로 답해주세요.', 'customer-signal'),
  scale('B02', 'B. 고객, 사용자, 내부 이해관계자 피드백 루프', '실제 사용자 신호 전달', '실제 이용자인 학생, 대학원생, 연구자의 사용 행동과 불편이 팀에 충분히 전달된다.', '현장 피드백, 사용 데이터, 문의, 인터뷰 등 실제 사용자의 신호를 접할 수 있는지 봅니다.', 'user-signal'),
  scale('B03', 'B. 고객, 사용자, 내부 이해관계자 피드백 루프', '내부 피드백 정리', '영업, 마케팅, 서비스, 콘텐츠 관련 조직에서 들어오는 피드백이 정리되어 우선순위 결정에 사용된다.', '요청이 단순 전달되는 수준을 넘어, 제품 판단에 쓸 수 있는 형태로 정리되는지 기준으로 답해주세요.', 'stakeholder-loop'),
  scale('B04', 'B. 고객, 사용자, 내부 이해관계자 피드백 루프', '충돌 신호 판단 기준', '사용자 문의, 불만, 사용 데이터, 영업 현장의 이야기가 서로 다를 때 판단 기준이 있다.', '서로 다른 신호가 충돌할 때 어떤 신호를 더 중시할지 논의하거나 결정하는 방식이 있는지 봅니다.', 'signal-conflict'),
  ...
  scale('E04', 'E. 다직군 협업 인터페이스', '디자인/UX 조기 반영', '디자인/UX 관점이 기능 후반이 아니라 문제 정의와 설계 초기에 반영된다.', '디자인 산출물 자체보다 사용자 경험 관점이 너무 늦게 들어오지 않는지 봅니다.', 'ux-collaboration'),
  scale('E05', 'E. 다직군 협업 인터페이스', '인프라/운영 조기 확인', '인프라/운영 관련 제약은 일정 후반이 아니라 충분히 이른 시점에 확인된다.', '배포, 성능, 비용, 보안, 모니터링 같은 제약이 늦게 드러나는지 기준으로 답해주세요.', 'infra-collaboration'),
  ...
  scale('F02', 'F. 제품별 실행 방식과 성숙도 관리', '소규모 전담 공유', 'AI Editor처럼 소규모 전담으로 진행되는 워크스트림도 필요한 수준으로 팀에 공유된다.', '모든 세부 진행을 공유해야 한다는 뜻은 아닙니다. 팀 정렬에 필요한 정보가 공유되는지 기준으로 답해주세요.', 'cbt-sharing'),
  ...
  scale('G01', 'G. 개발/운영 경험', '로그/모니터링 접근성', '개발자가 필요한 로그, 모니터링, 재현 정보에 접근해 문제를 빠르게 파악할 수 있다.', '장애나 품질 이슈가 있을 때 원인 파악에 필요한 정보 접근성이 충분한지 답해주세요.', 'observability'),
  scale('G02', 'G. 개발/운영 경험', '배포 검증과 속도', '배포 또는 릴리즈 과정은 필요한 검증을 포함하면서도 과도하게 느리지 않다.', '속도만 보지 말고, 검증과 실행 속도의 균형을 기준으로 답해주세요.', 'release-flow'),
  scale('G03', 'G. 개발/운영 경험', '변경 영향 범위', '공통 코드, 데이터 파이프라인, 검색/AI 구성요소의 변경 영향 범위를 이해할 수 있다.', '변경 시 어떤 제품이나 기능에 영향을 줄지 추적 가능한지 기준으로 답해주세요.', 'change-impact'),
  ...
  scale('G05', 'G. 개발/운영 경험', '장애/품질 대응 경로', '장애나 품질 문제가 발생했을 때 대응 역할과 커뮤니케이션 경로가 명확하다.', '문제가 생긴 뒤 누가 무엇을 확인하고 누구에게 알릴지 대체로 분명한지 답해주세요.', 'incident-response'),
