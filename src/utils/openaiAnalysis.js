const REPORT_VERSION = '2026-05-13-browser-analysis-v10-readable-empathy';
...
6. ## 5. 다음에 확인할 것
  ...
  letter: {
    title: '한 장의 편지',
    goal:
      '모든 문항을 망라하되, 긴 진단서가 아니라 팀원이 한 번에 읽을 수 있는 따뜻한 한 장의 편지를 작성하세요. 핵심 흐름과 우선 논의할 문제를 공감과 배려가 느껴지는 쉬운 언어로 정리하세요.',
    requiredSections: `1. # 한 장의 편지
   - 본문은 3~5개 문단으로만 작성하세요. 목록을 남발하지 말고, 편하게 읽히는 짧은 문단으로 쓰세요.
   - 각 문단의 첫 문장은 반드시 **굵은 글씨**로 쓰세요.
   - 각 문단의 첫 문장은 직접 요약문처럼 쓰지 말고, 비유나 말 건넴을 통해 문단의 핵심 감정과 흐름이 한눈에 보이게 쓰세요.
   - 예: "**속도를 내는 배일수록, 서로 보는 나침반이 더 중요해집니다.**"처럼 간접적이지만 뜻이 잡히는 문장으로 쓰세요.
   - 각 문단의 첫 문장은 18단어 이내로 짧게 쓰되, 단정적인 보고서 제목처럼 쓰지 마세요.
   - 각 문단은 첫 문장 포함 3~5문장으로 제한하세요.
   - 말투는 따뜻한 마음씨를 가진 상담자가 팀에게 조심스럽게 말을 건네는 편지처럼 쓰세요.
   - "~하느라 마음이 많이 쓰였을 것 같습니다", "~를 붙잡고 오느라 고생이 많으셨을 것 같습니다"처럼 응답 신호와 연결된 공감을 자연스럽게 포함하세요.
  ...
  roleMessages: {
   ...
   - 첫 두 문장은 반드시 응답 기반 공감으로 시작하세요. "~하느라 고생이 많으셨을 것 같습니다", "~하신 것 같아 마음이 쓰입니다" 같은 자연스러운 공감 표현을 포함하세요.
   ...
   - 이모티콘은 사용하지 마세요.
   - 형식 예시:
     > **PM/제품 담당 분들께:** "우선순위와 고객 맥락을 계속 연결하느라, 매번 설명하고 다시 정리해야 하는 시간이 적지 않았을 것 같습니다. 여러 요청 사이에서 무엇을 먼저 해야 할지 붙잡고 계셨을 모습을 떠올리면 마음이 쓰입니다. 제품/PM/프롬프트 응답에서 결정 기준과 기술 제약 정보가 함께 보일 때 일이 편해진다는 신호가 보인다면, 문제는 개인의 설득력이 아니라 기준이 아직 충분히 공유되지 않은 데 있을 수 있습니다. 다음 4주는 제품별로 이번 달 할 일과 하지 않을 일을 짧게 고정해두면, 설명의 부담을 조금 덜 수 있겠습니다."
     > **AI 엔지니어링 분들께:** "품질, 검색, 응답 속도, 운영 안정 사이에서 정답이 하나로 떨어지지 않는 문제를 계속 붙잡고 있었을 것 같습니다. AI 변경 결과와 기록에 대한 응답 신호는 실험 자체보다, 실험에서 배운 점이 제품 결정으로 넘어가는 길을 더 잘 보이게 해야 한다는 쪽에 가깝습니다. 먼저 핵심 제품 한두 개에만 작은 평가 기준표를 붙여보면, 판단의 부담이 조금 나뉠 수 있겠습니다."
     > **프론트엔드 분들께:** "사용자가 마지막으로 만나는 화면에서 지연, 실패, 안내 문구까지 받아내느라 생각할 것이 많았을 것 같습니다. 웹 개발 응답에서 구현 전 합의와 상태 처리 기준이 중요하게 보인다면, 화면 작업의 어려움은 단순 구현량보다 늦게 정해지는 기준에서 커질 수 있습니다. 주요 화면마다 PM, 디자인, 개발이 함께 보는 짧은 점검표를 두면 다시 맞추는 시간을 줄일 수 있겠습니다."`,
    roleMessages: true,
  },
  closedEnded: {
...
- ${isNarrativeMessage ? '핵심 문장은 문단 첫 문장이나 역할별 첫 문장에 자연스럽게 녹이세요.' : '이 세 문장은 포스터 헤드라인처럼 짧고 선명해야 합니다. 예: "일은 빠른데, 교통정리가 늦습니다."'}
...
- ${isNarrativeMessage ? '공감 표현은 반드시 설문 신호와 연결해 쓰세요. 데이터 없이 다 안다는 듯한 위로나 과한 칭찬은 피하세요.' : '상세 수치는 기본 결과 화면에서 확인할 수 있으므로, 수치표를 반복하기보다 의미와 제안을 중심으로 쓰세요.'}
