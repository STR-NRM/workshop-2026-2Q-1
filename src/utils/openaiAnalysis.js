const REPORT_VERSION = '2026-05-13-browser-analysis-v12-letter-warm-readable';
...
6. ## 5. 다음에 확인할 것
  ...
  letter: {
    title: '한 장의 편지',
    goal:
      '모든 문항을 망라하되, 긴 진단서가 아니라 팀원이 한 번에 읽을 수 있는 따뜻한 한 장의 편지를 작성하세요. 따뜻하고 배려 있는 상담자처럼 공감하되, 응답 근거에 기반해 현재 흐름과 우선 논의할 문제를 쉬운 언어로 정리하세요.',
    requiredSections: `1. # 한 장의 편지
   - 본문은 3~5개 문단으로만 작성하세요. 목록을 남발하지 말고, 편하게 읽히는 짧은 문단으로 쓰세요.
   - 각 문단의 첫 문장은 반드시 **굵은 글씨**로 쓰세요.
   - 각 문단의 첫 문장은 보고서 제목이 아니라, 팀에게 조심스럽게 말을 건네는 쉬운 핵심 문장이어야 합니다.
   - 각 문단의 첫 문장만 이어 읽어도 전체 흐름이 잡혀야 합니다. 단, 암호처럼 느껴지는 추상적 비유만 쓰면 안 됩니다.
   - 첫 문장에는 가능한 한 우선순위, 제품, 협업, 일정, 품질, 운영, 실험 같은 실제 업무 단어를 자연스럽게 넣으세요.
   - 좋은 첫 문장 예: "**지금 스쿼드는 느린 팀이라서가 아니라, 동시에 너무 많은 방향을 붙잡고 있어서 흔들리고 있습니다.**"
   - 좋은 첫 문장 예: "**우선순위가 자주 접히면, 열심히 움직인 사람일수록 더 쉽게 지칩니다.**"
   - 피해야 할 첫 문장 예: "**흐린 항로표 위에 접힌 등대가 조용히 흔들립니다.**"
   - 각 문단의 첫 문장은 24단어 이내로 짧게 쓰되, 누구나 바로 알아들을 수 있어야 합니다.
   - 각 문단은 첫 문장 포함 3~5문장으로 제한하세요.
   - 말투는 따뜻하고 배려 깊고 정중한 상담자가 팀에게 조심스럽게 말을 건네는 편지처럼 쓰세요.
   - 공감은 반드시 설문 신호와 연결하세요. 특정 위로 문구를 반복하지 말고, 문단마다 상황에 맞는 다른 말로 자연스럽게 표현하세요.
   - 참고할 수 있는 공감의 방향: "여러 일을 동시에 붙잡느라 숨이 찼겠어요", "기준이 늦게 정해질 때 다시 맞춰야 하는 부담이 있었겠어요", "품질과 속도 사이에서 조심스럽게 판단해야 했겠어요". 이 문장을 그대로 복사하지 말고 응답 내용에 맞게 새로 쓰세요.
   - 자연스러운 한국어를 사용하고, 딱딱한 평가서, 과한 칭찬, 과한 위로, 특정 직군 탓으로 읽히는 표현은 피하세요.
   - 비유는 쓰면 좋습니다. 다만 이해를 돕는 쉬운 비유만 1~2개 정도 사용하고, 모든 문단 첫 문장을 비유로 채우지 마세요.
   - 비유를 썼다면 같은 문장이나 다음 문장에서 실제 업무 의미를 바로 풀어주세요.
   - 수치, 선택형 신호, 서술형 신호, 역할별 추가 문항의 공통 흐름을 자연스럽게 연결하되, 데이터가 약한 부분은 조심스럽게 표현하세요.
   - 데이터 인용은 자연스럽게 1~2회만 사용하세요. 문항 ID나 긴 수치표처럼 딱딱한 표기는 피하세요.
   - "보다 보니", "그런데", "다시 말하면", "마지막으로 한 가지" 같은 대화형 연결어를 자연스럽게 사용할 수 있습니다. 단, 반복하지 마세요.
   - 이 문서는 오직 한 장의 편지만 담습니다. Executive Summary, 한 문장 결론, 역할별 메시지, 표, 실행 후보 목록, 토론 질문 목록은 작성하지 마세요.`,
    letter: true,
  },
  roleMessages: {
    title: '직무별 메시지',
    goal:
      '역할별 응답 신호와 공통 운영 신호를 바탕으로, 각 직무의 팀원에게 따뜻하고 조심스럽게 건네는 짧은 메시지를 작성하세요. 각 메시지는 개인 평가가 아니라 그 역할의 업무 경험을 이해하고 위로하는 말이어야 합니다.',
   ...
   - 각 역할 메시지는 4~6문장으로 작성하세요.
   - 첫 문단의 흐름은 "그 역할이 실제로 겪었을 법한 상황에 대한 공감" → "응답에서 보이는 조심스러운 신호" → "작게 해볼 제안 또는 격려" 순서로 이어가세요.
   ...
   - 중간 문장은 조심스러운 진단과 응답 신호를 연결하고, 마지막 문장은 작은 제안 또는 격려로 작성하세요.
   - 한 역할 메시지는 300~520자 정도로 쓰되, 불필요하게 길게 늘이지 마세요.
   - 말투는 따뜻한 마음씨를 가진 상담자가 조용히 말을 건네는 느낌으로 쓰세요. 다만 실제 응답 근거가 없는 위로나 칭찬은 하지 마세요.
   - "고생하셨습니다"처럼 포괄적인 격려만 반복하지 말고, 각 역할이 실제로 겪었을 법한 부담과 응답 신호를 연결해 구체적으로 공감하세요.
   - 설문 메타 역할은 제품/PM/프롬프트, AI 엔지니어링, 웹 개발(프론트/백엔드) 세 묶음입니다.
   - PM과 프롬프트, 프론트엔드와 백엔드는 별도 응답값이 없으므로, 분리해서 말할 때는 "제품/PM/프롬프트 응답에서 보이는 흐름", "웹 개발 응답에서 보이는 흐름"처럼 조심스럽게 표현하세요.
   - 절대 개별 직무의 별도 평균이나 별도 다수 의견처럼 단정하지 마세요.
   - 이모티콘은 사용하지 마세요.
   - 형식 예시:
     > **PM/제품 담당 분들께:** "우선순위, 고객 맥락, 회사 안 다른 팀과의 협업 흐름을 동시에 붙들다 보면 매주 방향을 다시 설명해야 하는 시간이 많았겠어요. 제품/PM/프롬프트 응답에서 결정 기준과 기술 제약 정보가 함께 보일 때 일이 편해진다는 신호가 보인다면, 문제는 개인의 설득력이 아니라 기준이 아직 충분히 공유되지 않은 데 있을 수 있습니다. 그래도 작은 결정이 빠르게 움직인 흔적이 함께 보인다면, 연결하는 일이 이미 팀의 혈관처럼 작동하고 있다는 뜻입니다. 다음 4주는 제품별로 이번 주 할 일, 하지 않을 일, 결정이 필요한 일을 짧게 고정해두면 설명의 부담을 조금 덜 수 있겠습니다."
     > **AI 엔지니어링 분들께:** "품질, 검색, 응답 속도, 운영 안정 사이에서 정답이 하나로 떨어지지 않는 문제를 오래 들여다보고 계셨겠어요. AI 변경 결과와 기록에 대한 응답 신호는 실험 자체보다, 실험에서 배운 점이 제품 결정으로 넘어가는 길을 더 잘 보이게 해야 한다는 쪽에 가깝습니다. 동시에 모델과 검색 흐름을 비교적 빠르게 바꿔볼 수 있는 힘도 보인다면, 그 힘은 앞으로 더 중요한 기반이 될 수 있습니다. 먼저 핵심 제품 한두 개에만 작은 평가 기준표를 붙여보면 판단의 부담이 조금 나뉠 수 있겠습니다."
     > **프론트엔드 분들께:** "사용자가 마지막으로 만나는 화면에서 일정, 디자인, 상태 처리, 안내 문구를 함께 맞추다 보면 다시 조율해야 할 일이 적지 않았겠어요. 웹 개발 응답에서 구현 전 합의와 상태 처리 기준이 중요하게 보인다면, 화면 작업의 어려움은 단순 구현량보다 늦게 정해지는 기준에서 커질 수 있습니다. 그럼에도 화면을 끝까지 다듬는 일은 제품 신뢰를 만드는 가장 가까운 접점입니다. 주요 화면마다 PM, 디자인, 개발이 함께 보는 짧은 점검표를 두면 다시 맞추는 시간을 줄일 수 있겠습니다."`,
    roleMessages: true,
  },
  closedEnded: {
...
function buildPrompt(payload, analysisType) {
  ...
  const structureRule = isLetter
    ? '- 편지는 긴 보고서 문법보다 읽히는 흐름이 우선입니다. 각 문단은 공감, 관찰, 해석, 제안이 자연스럽게 이어지게 쓰세요.'
    : isRoleMessages
      ? '- 직무별 메시지는 역할별 업무 경험을 이해하기 위한 짧은 말입니다. 진단보다 공감, 단정보다 조심스러운 제안을 우선하세요.'
...
- 상세 리포트는 충분히 길고 구조적이어야 하지만, 각 섹션의 핵심은 위 두 문장만 읽어도 파악되어야 합니다.`;
  const closingRule = isLetter
    ? '편지에는 별도의 "4주 개선 후보" 상세 양식을 붙이지 마세요. 필요한 제안은 본문 안에 짧게 녹이세요.'
    : isRoleMessages
      ? '직무별 메시지에는 별도의 "4주 개선 후보" 상세 양식을 붙이지 마세요. 각 역할의 마지막 문장에 작고 실행 가능한 제안을 자연스럽게 담으세요.'
