const REPORT_VERSION = '2026-05-13-browser-analysis-v14-role-group-voice';
...
6. ## 5. 다음에 확인할 것
  ...
  letter: {
   ...
   - 편지는 제안보다 공감과 편지 말투의 가벼운 해석이 더 중요합니다. 제안을 각 문단마다 넣지 마세요.
   - 앞의 문단들은 "어떤 마음이 있었을지", "무엇이 조금 힘들었을지", "그래도 어떤 힘이 남아 있는지"를 편지처럼 조심스럽게 읽어주세요.
   ...
   - 마지막 문단에서만 조심스럽게 제안을 건네세요. 전문 용어 없이 "조금 덜 흔들리게 같이 맞춰가면 좋겠습니다"처럼 말하고, 마지막 문장은 다시 공감과 격려로 닫으세요.
  ...
  roleMessages: {
   ...
   - 응답 데이터에 roleGroupContexts가 있으면 가장 먼저 읽으세요. 직무별 전용 문항뿐 아니라, 그 직무 그룹이 공통 문항과 서술형에서 어떤 마음과 흐름을 보였는지도 함께 반영하세요.
   - roleGroupContexts의 commonResponseSignals는 숫자가 아니라 방향을 잡기 위한 참고 메모입니다. 본문에 "어려움을 느끼는 쪽", "자주 보인 선택" 같은 메모 표현을 그대로 쓰지 말고 일상 언어로 바꾸세요.
   - roleGroupContexts의 textResponses는 각 직무 그룹 구성원이 직접 남긴 목소리입니다. 표현을 그대로 길게 옮기지 말고, 마음의 결을 살려 조심스럽게 반영하세요.
...
function buildPrompt(payload, analysisType) {
  ...
  const structureRule = isLetter
    ? '- 편지는 설명문보다 마음의 흐름이 우선입니다. 앞 문단은 공감과 편지 말투의 가벼운 해석을 중심에 두고, 제안은 마지막 문단에서만 조심스럽게 건네세요.'
    : isRoleMessages
  ...
  const workshopActionRule = isLetter
    ? '- 편지에는 별도 "워크샵에서 확인할 질문" 목록이나 4주 개선 후보 상세 목록을 넣지 마세요. 제안은 마지막 문단에만 한두 문장으로 조심스럽게 녹이세요.'
    : isRoleMessages
...
- 상세 리포트는 충분히 길고 구조적이어야 하지만, 각 섹션의 핵심은 위 두 문장만 읽어도 파악되어야 합니다.`;
  const closingRule = isLetter
    ? '편지에는 별도의 "4주 개선 후보" 상세 양식을 붙이지 마세요. 마지막 문단에서 작은 바람을 건넨 뒤, 다시 공감과 격려로 닫으세요.'
    : isRoleMessages
...
- 확신 수준은 높음/중간/낮음으로 표시하고, 확신 수준이 낮은 결론은 실행안으로 단정하지 마세요.`;
  const flowRule = isNarrativeMessage
    ? isLetter
      ? '- 편지의 흐름은 마음을 알아주는 말, 함께 본 장면, 편지 말투의 가벼운 해석, 마지막의 작은 바람, 다시 공감과 격려 순서로 자연스럽게 이어지게 하세요.'
