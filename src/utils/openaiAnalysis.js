const REPORT_VERSION = '2026-05-13-browser-analysis-v5-readable-report-suite';
...
const reportSpecs = {
  comprehensive: {
   ...
   - 이어서 "## 쉬운 분석 요약"을 만들고, 설문 결과를 쉬운 말로 3~5개 bullet로 풀어쓰세요.
   - 이어서 "## 그래서 무엇을 해야 하나"를 만들고, 바로 실행할 제안 3~5개를 bullet로 작성하세요.
   - 각 bullet은 18단어 이내로 짧게 쓰고, 가능한 경우 근거 수치 또는 관찰 신호를 붙이세요.
2. ## 1. 데이터 범위와 해석 주의
...
9. ## 8. 다음 설문 또는 추가 확인 항목
  ...
  closedEnded: {
    ...
    requiredSections: `1. # Executive Summary
   - 첫 줄은 반드시 **한 문장 결론:** "핵심 문장" 형식으로 작성하세요. 따옴표 안은 10단어 이내여야 합니다.
   - 이어서 "## 쉬운 분석 요약"을 만들고, 정량 문항에서 보이는 핵심 신호 3~5개를 쉬운 한국어 bullet로 요약하세요.
   - 이어서 "## 그래서 무엇을 해야 하나"를 만들고, 정량 결과만 보고도 시작할 수 있는 제안 3~5개를 bullet로 작성하세요.
   - 각 bullet은 18단어 이내로 짧게 쓰세요.
2. ## 1. 정량 데이터 범위와 한계
...
7. ## 6. 4주 실행 실험 후보
  ...
  textByQuestion: {
    ...
    requiredSections: `1. # Executive Summary
   - 첫 줄은 반드시 **한 문장 결론:** "핵심 문장" 형식으로 작성하세요. 따옴표 안은 10단어 이내여야 합니다.
   - 이어서 "## 쉬운 분석 요약"을 만들고, 주관식 응답 전체에서 보이는 핵심 테마 3~5개를 쉬운 한국어 bullet로 요약하세요.
...
- 독자가 10초 안에 핵심을 잡아야 합니다. 긴 개념어, 컨설팅 용어, 추상명사는 줄이고 동사 중심으로 쓰세요.
