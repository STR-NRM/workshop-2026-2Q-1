const REPORT_VERSION = '2026-05-13-browser-analysis-v8-warmer-letter';
...
9. ## 8. 다음 설문 또는 추가 확인 항목
  ...
  letter: {
    title: '편지',
    goal:
      '모든 문항을 망라하되, 긴 진단서가 아니라 팀원이 한 번에 읽을 수 있는 편지형 분석을 작성하세요. 핵심 흐름, 우선 논의할 문제, 직무별로 건넬 말을 따뜻하고 쉬운 언어로 정리하세요.',
    requiredSections: `1. # Executive Summary
   - 첫 줄은 반드시 **한 문장 결론:** "핵심 문장" 형식으로 작성하세요. 따옴표 안은 10단어 이내여야 합니다.
   ...
   - 말투는 차분한 전문가가 팀에게 건네는 편지처럼 쓰세요. 딱딱한 평가서, 과한 칭찬, 과한 위로, 특정 직군 탓으로 읽히는 표현은 피하세요.
   - 쉬운 비유를 1~2개까지 사용할 수 있습니다. 단, 과장하거나 특정 직군을 탓하는 비유는 쓰지 마세요.
   - 수치, 선택형 신호, 서술형 신호, 직무별 추가 문항을 자연스럽게 연결하되, 데이터가 약한 부분은 조심스럽게 표현하세요.
2. ## 직무별로 건네는 말
   - 설문 메타 역할은 제품/PM/프롬프트, AI 엔지니어링, 웹 개발(프론트/백엔드) 세 묶음입니다. 프론트엔드와 백엔드는 별도 응답값이 없으므로, 웹 개발 신호를 근거로 조심스럽게 나누어 말하세요.
   - 역할은 가능한 한 이 순서로 작성하세요: PM에게, 프롬프트 엔지니어에게, AI 엔지니어링에게, 프론트엔드에게, 백엔드에게.
   ...
   - 한 역할 메시지는 180~280자 정도로 쓰되, 길게 늘이지 마세요.
   - 따뜻하게 말하되, 칭찬이나 위로를 과하게 쓰지 마세요. 실제 응답 근거가 없는 말은 하지 마세요.
   - 이모티콘은 사용하지 마세요.
   - 형식 예시:
     > **PM에게:** "우선순위와 고객 맥락을 계속 연결하느라 설명해야 할 일이 많았을 것 같습니다. 응답에서 방향과 결정 기준이 흔들리는 신호가 보인다면, 그것은 개인 역량보다 기준이 아직 충분히 보이지 않는 문제일 수 있습니다. 다음 4주는 제품별로 이번 달 할 일과 하지 않을 일을 짧게 고정해보면 좋겠습니다."
     > **AI 엔지니어링에게:** "품질, 검색, 응답 속도, 운영 안정 사이에서 답이 하나로 떨어지지 않는 문제를 계속 붙잡고 있었을 것 같습니다. 실험 결과가 결정으로 이어지는 길이 길어 보인다면, 바꾼 것과 배운 것을 더 쉽게 공유할 장치가 필요합니다. 먼저 핵심 제품 한두 개에만 작은 평가 기준표를 붙여보면 좋겠습니다."
     > **프론트엔드에게:** "사용자가 마지막으로 만나는 화면에서 AI 기능의 지연, 실패, 안내까지 받아내느라 고려할 것이 많았을 것 같습니다. 구현 전 합의가 늦으면 화면에서 다시 맞추는 비용이 커질 수 있습니다. 주요 화면마다 PM, 디자인, 개발이 함께 보는 짧은 점검표를 써보면 좋겠습니다."`,
    letter: true,
  },
  closedEnded: {
