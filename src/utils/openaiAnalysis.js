const REPORT_VERSION = '2026-05-13-browser-analysis-v9-letter-role-split';
...
9. ## 8. 다음 설문 또는 추가 확인 항목
  ...
  letter: {
    title: '한 장의 편지',
    goal:
      '모든 문항을 망라하되, 긴 진단서가 아니라 팀원이 한 번에 읽을 수 있는 한 장의 편지를 작성하세요. 핵심 흐름과 우선 논의할 문제를 따뜻하고 쉬운 언어로 정리하세요.',
    requiredSections: `1. # Executive Summary
   - 첫 줄은 반드시 **한 문장 결론:** "핵심 문장" 형식으로 작성하세요. 따옴표 안은 10단어 이내여야 합니다.
   - 이어서 "## 한 장의 편지"를 작성하세요.
   ...
   - 말투는 따뜻하고 배려 깊은 상담자가 팀에게 조심스럽게 말을 건네는 편지처럼 쓰세요.
   ...
   - 이 문서는 오직 한 장의 편지만 담습니다. 역할별 메시지, 표, 실행 후보 목록, 토론 질문 목록은 작성하지 마세요.`,
    letter: true,
  },
  roleMessages: {
    title: '직무별 메시지',
    goal:
      '역할별 응답 신호와 공통 운영 신호를 바탕으로, 각 직무의 팀원에게 따뜻하고 조심스럽게 건네는 짧은 메시지를 작성하세요.',
    requiredSections: `1. # Executive Summary
   - 첫 줄은 반드시 **한 문장 결론:** "핵심 문장" 형식으로 작성하세요. 따옴표 안은 10단어 이내여야 합니다.
   - 이어서 "## 직무별 메시지를 읽는 법"을 작성하고, 이 메시지가 개인 평가가 아니라 역할별 업무 경험을 이해하기 위한 보조 자료임을 2~3문장으로 설명하세요.
2. ## 직무별 메시지
   ...
   - 각 역할 메시지는 3~5문장으로 작성하세요.
   - 첫 문장은 응답 기반 공감, 중간 문장은 조심스러운 진단과 근거, 마지막 문장은 작은 제안 또는 격려로 작성하세요.
   - 한 역할 메시지는 240~420자 정도로 쓰되, 불필요하게 길게 늘이지 마세요.
   - 말투는 따뜻하고 배려 깊은 상담자가 조용히 말을 건네는 느낌으로 쓰세요. 다만 실제 응답 근거가 없는 위로나 칭찬은 하지 마세요.
   - "고생하셨습니다"처럼 포괄적인 격려만 반복하지 말고, 응답 신호와 연결된 구체적인 공감을 쓰세요.
   ...
   - 이모티콘은 사용하지 마세요.
   - 형식 예시:
     > **PM/제품 담당 분들께:** "우선순위와 고객 맥락을 계속 연결하느라, 매번 설명하고 다시 정리해야 하는 시간이 적지 않았을 것 같습니다. 제품/PM/프롬프트 응답에서 결정 기준과 기술 제약 정보가 함께 보일 때 일이 편해진다는 신호가 보인다면, 문제는 개인의 설득력이 아니라 기준이 아직 충분히 공유되지 않은 데 있을 수 있습니다. 다음 4주는 제품별로 이번 달 할 일과 하지 않을 일을 짧게 고정해두면, 설명의 부담을 조금 덜 수 있겠습니다."
...
function buildPrompt(payload, analysisType) {
  ...
  const executiveSummaryRule = isLetter
    ? '- Executive Summary에는 첫 줄 한 문장 결론만 두고, 별도의 요약 목록은 넣지 마세요. 핵심 설명은 "한 장의 편지"에 담으세요.'
    : isRoleMessages
      ? '- Executive Summary는 길게 쓰지 마세요. 한 문장 결론과 역할별 메시지를 읽는 법만 간단히 넣으세요.'
...
- Executive Summary 이후에 **한문장 정리:**, **한문장 제안:** 형식을 반복하지 마세요.
