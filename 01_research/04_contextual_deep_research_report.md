...
7. 빠른 실험과 안정적 운영 사이의 균형를 명시적으로 결정하는가?
...
| 약 10명 규모 | 현재 실무 스쿼드는 15명 | 팀 크기 증가에 따른 동기화/오너십/동시에 진행 중인 일 진단 |
...
- Agent, Viewer, Reader, Idea, 공통 검색/RAG/플랫폼 같은 업무 영역별 Driver가 필요할 수 있다.
...
- 실무 스쿼드 15명은 two-pizza team의 작은 팀 원칙보다 큰 편이므로, 전체 스쿼드 안에 제품/플랫폼 업무 영역 단위가 필요할 수 있다.
...
### 3.6 GitLab Handbook / 책임 역할 / 비동기 커뮤니케이션

GitLab은 책임 역할, 문서화, 비동기 커뮤니케이션, 온보딩 체계를 강하게 운영한다.
...
Agent, Viewer, Idea, Reader, Editor가 동시에 움직이면서 우선순위와 동시에 진행 중인 일가 불명확해질 수 있다. 특히 AI Editor는 CBT 단계이고 소규모 전담 방식이므로, 정식 운영 제품과 같은 방식으로만 비교하면 해석이 왜곡될 수 있다.
