...
| 설문 컴포넌트 | 높음 | 척도, 선택, 서술형 컴포넌트 재사용 가능 |
...
- `multiChoice`: 복수 선택형
- `역할 선택`: 역할에 따라 선택 문항 노출
- `scale5na`: 5점 척도 + 해당 없음/판단하기 어려움
- `breakoutText`: 분과별 그룹 입력
- `도움말`: 질문 하단 설명문 렌더링용 필드

이번 설문은 시간이 30분 이상 확보되어 있고 직군별 문항이 필요하므로 `역할 선택`, `도움말`, `scale5na`, 조건부 라우팅은 구현 우선순위가 높다.
