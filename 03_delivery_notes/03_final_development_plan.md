...
- 최초 작성일자: 2026-05-13
- 업데이트일자: 2026-05-13
- 업데이트 내용: GitHub Pages 정적 배포, Firebase Realtime Database 저장, 런타임 OpenAI key 기반 AI 분석으로 최종 구조 재정리.
- 작성자: Codex
...
4. 따라서 최종 구조는 정적 앱 + Firebase RTDB + 런타임 OpenAI key 입력 방식으로 정했다. 응답자는 추가 설정 없이 설문을 제출하고, 진행자만 결과 화면에서 key를 입력해 AI 분석을 생성한다.
5. AI 분석 결과는 Firebase에 저장해 팀원이 같은 결과 페이지에서 읽을 수 있게 한다. key 자체는 소스, GitHub secret, Firebase 어디에도 저장하지 않는다.
...
| AI 분석 | 결과 페이지에서 OpenAI Responses API 직접 호출 | Functions/Blaze 없이 클릭 기반 분석 가능 |
...
4. 진행자가 결과 페이지의 `AI 리포트` 탭에서 OpenAI API key를 입력하고 `AI 분석 생성/갱신`을 누른다.
5. 브라우저가 익명 집계 payload를 OpenAI Responses API로 보내고, 받은 보고서를 `/surveys/2026-2Q-1/analysis/comprehensive`에 저장한다.
6. 이후 팀원은 같은 결과 페이지에서 저장된 AI 리포트를 조회한다.
...
7. 워크샵 진행자가 AI 리포트를 한 번 생성하고 결과가 Firebase에 저장되는지 확인한다.
