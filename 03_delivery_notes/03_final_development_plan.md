...
- 최초 작성일자: 2026-05-13
- 업데이트일자: 2026-05-13
- 업데이트 내용: GitHub Pages 정적 배포, Firebase Realtime Database 저장, 런타임 OpenAI key 기반 AI 분석과 종합/선택형/서술형 리포트 동시 생성 구조로 최종 구조 재정리.
- 작성자: Codex
...
5. AI 분석 결과는 종합, 선택형, 서술형 문항별 리포트로 나누어 Firebase에 저장한다. 어느 리포트 탭에서 생성 버튼을 눌러도 3개가 동시에 생성되며, key 자체는 소스, GitHub secret, Firebase 어디에도 저장하지 않는다.
...
| 인증 | Firebase Anonymous Auth | 응답 저장 권한을 사용자별 저장 단위로 제한 |
| 결과 조회 | 공개 조회 | 팀 워크샵 목적상 로그인 제거 요구 반영 |
| AI 분석 | 결과 페이지의 3개 AI 리포트 탭에서 OpenAI Responses API 직접 호출 | Functions/Blaze 없이 3종 리포트를 동시에 생성 가능 |
| OpenAI key | 진행자 런타임 입력 | 번들/저장소/Firebase 노출 방지 |

## 3. 운영 흐름

1. 팀원이 GitHub Pages URL에 접속한다.
2. 앱이 Anonymous Auth 저장 단위를 만들고 `/surveys/2026-2Q-1/respondents/{uid}`와 `/responses/{uid}`에 저장한다.
3. 결과 페이지는 Firebase에서 응답, 응답 시작 기록, 기존 AI 분석 결과를 읽는다.
4. 진행자가 결과 페이지의 `AI 종합`, `AI 선택형`, `AI 서술형` 탭 중 아무 화면에서나 리포트 생성용 key를 입력하고 리포트 생성 버튼을 누른다.
5. 브라우저가 종합, 선택형, 서술형 payload를 OpenAI Responses API에 동시에 보내고, 받은 보고서를 `/surveys/2026-2Q-1/analysis/comprehensive/reports/...` 구조로 나누어 저장한다.
...
7. 워크샵 진행자가 AI 리포트 탭 중 하나에서 생성 버튼을 누르고, 종합, 선택형, 서술형 문항별 AI 리포트 3개가 모두 Firebase에 저장되는지 확인한다.
